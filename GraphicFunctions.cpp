...
void Draw_bottom_Gouraud_Triangle(float x1,float y1, float x2,float y2, float x3,float y3, Color color0,Color color1,Color color2, UCHAR *dest_buffer, int mempitch){
	...
	if(x1>=min_clip_x&&x1<=max_clip_x&&x2>=min_clip_x&&x2<=max_clip_x&&x3>=min_clip_x&&x3<=max_clip_x){
		...
		float clip_xs,clip_xe,clip_rs,clip_gs,clip_bs;
		...
		for (loopy = iy1;loopy<=iy3;loopy++,lp_addr+=mempitch)
		{
			...
			xs += left;
			xe += right;
			drdx = (rd-rs)/(clip_xe - clip_xs + 1);
			dgdx = (gd-gs)/(clip_xe - clip_xs + 1);
			dbdx = (bd-bs)/(clip_xe - clip_xs + 1);
			rs += dr_left;
			gs += dg_left;
			bs += db_left;
			rd += dr_right;
			...
			if (clip_xs<min_clip_x)
			{
				clip_rs += (min_clip_x - clip_xs)*drdx;
				clip_gs += (min_clip_x - clip_xs)*dgdx;
				clip_bs += (min_clip_x - clip_xs)*dbdx;
				clip_xs = min_clip_x;
			...
			{
				//loopx_dx = (loopx/dx);
				final_color = _RGB32BIT(0xff,(int)(clip_rs + loopx*drdx),(int)(clip_gs + loopx*dgdx),(int)(clip_bs + loopx*dbdx));
				Mem_Set_QUAD(lp_addr+ixs+loopx,final_color,1);
			}
		}		
	}
...
void Draw_Gouraud_Triangle_zb(RenderPoly* poly,UCHAR *dest_buffer, int mempitch,float* z_buffer,int z_pitch){
	...
	case TRI_TYPE_BOTTOM:
		...
		if(x1>=min_clip_x&&x1<=max_clip_x&&x2>=min_clip_x&&x2<=max_clip_x&&x3>=min_clip_x&&x3<=max_clip_x){
			...
			float clip_xs,clip_xe,clip_zs,clip_ze,clip_rs,clip_gs,clip_bs;
			float drdx,dgdx,dbdx;
...
void drawTextureTriangle_zb_gouraud(RenderPoly *poly,BitmapData *texture,UCHAR *dest_buffer, int mempitch,float* z_buffer,int z_pitch){
	...
	float clip_xs,clip_xe,clip_zs,clip_ze,clip_txs,clip_tys,clip_rs,clip_gs,clip_bs;
	float drdx,dgdx,dbdx;
	switch(tri_type){
	case TRI_TYPE_BOTTOM:
		y21 = 1/(y2-y1);
		...
		if(x1>=min_clip_x&&x1<=max_clip_x&&x2>=min_clip_x&&x2<=max_clip_x&&x3>=min_clip_x&&x3<=max_clip_x){
			for (loopy = iy1;loopy<=iy3;loopy++,lp_addr+=mempitch,lp_zaddr+=z_pitch)
			{
				...
				for (loopx = 0;loopx<dx;loopx++)
				{
					...
					if (current_z < *(lp_zaddr+ixs+loopx))
					{
						loopx_dx = (loopx/dx);
						//final_color = _RGB32BIT(0xff,(int)(rs + (rd-rs)*loopx_dx),(int)(gs + (gd-gs)*loopx_dx),(int)(bs + (bd-bs)*loopx_dx));
						light_r = (rs + (rd-rs)*loopx_dx)/256;
						light_g = (gs + (gd-gs)*loopx_dx)/256;
						light_b = (bs + (bd-bs)*loopx_dx)/256;
						texture_x = (txs + loopx*tx_step)*(texture->width - 1);
						texture_y = (tys + loopx*ty_step)*(texture->height - 1);
						temp_color = texture->data[(texture_y<<texture->power_of_two)+texture_x];
						temp_color.r *= light_r;
			...
			for (loopy = iy1;loopy<=iy3;loopy++,lp_addr+=mempitch,lp_zaddr+=z_pitch)
			{
				...
				for (loopx = 0;loopx<(int)dx;loopx++)
				{
					...
					{
						//loopx_dx = (loopx/dx);
						light_r = (clip_rs + loopx*drdx)/256;
						light_g = (clip_gs + loopx*dgdx)/256;
						light_b = (clip_bs + loopx*dbdx)/256;
						texture_x = (clip_txs + loopx*tx_step)*(texture->width - 1);
						texture_y = (clip_tys + loopx*ty_step)*(texture->height - 1);
						temp_color = texture->data[(texture_y<<texture->power_of_two)+texture_x];
						temp_color.r *= light_r;
...
void Draw_Triangle_zb_alpha(RenderPoly* poly,UCHAR *dest_buffer, int mempitch,float* z_buffer,int z_pitch,UINT* alpha_table){
	Vertex3d data1;
	Vertex3d data2;
	Vertex3d data3;
	Vertex3d *v1;
		...
		if(x1>=min_clip_x&&x1<=max_clip_x&&x2>=min_clip_x&&x2<=max_clip_x&&x3>=min_clip_x&&x3<=max_clip_x){
			for (loopy = iy1;loopy<=iy3;loopy++,lp_addr+=mempitch,lp_zaddr+=z_pitch)
			{
				...
				for (loopx = 0;loopx<(int)dx;loopx++)
				{
					...
					if (current_z < *(lp_zaddr+ixs+loopx))
					{
						final_color.argb = alpha_table[poly->lit_color[0].argb&0x00ffffff] + alpha_table[*(lp_addr+ixs+loopx)&0x00ffffff];
						Mem_Set_QUAD(lp_addr+ixs+loopx,final_color.argb,1);
						//*(lp_zaddr+ixs+loopx) = current_z;
					}
				}
				xs += dxdyl;
			...
			for (loopy = iy1;loopy<=iy3;loopy++,lp_addr+=mempitch,lp_zaddr+=z_pitch)
			{
				...
				for (loopx = 0;loopx<dx;loopx++)
				{
					...
					if (current_z < *(lp_zaddr+ixs+loopx))
					{
						loopx_dx = (loopx/dx);
						final_color = _RGB32BIT(0xff,(int)(rs + (rd-rs)*loopx_dx),(int)(gs + (gd-gs)*loopx_dx),(int)(bs + (bd-bs)*loopx_dx));
						final_color = alpha_table[final_color&0x00ffffff] + alpha_table[*(lp_addr+ixs+loopx)&0x00ffffff];
						Mem_Set_QUAD(lp_addr+ixs+loopx,final_color,1);
						//*(lp_zaddr+ixs+loopx) = current_z;
					}					
				}
...
void drawTextureTriangle_invzb(RenderPoly *poly,BitmapData *texture,UCHAR *dest_buffer, int mempitch,float* z_buffer,int z_pitch){
	...
	float x1,y1,x2,y2,x3,y3,tx1,tx2,tx3,ty1,ty2,ty3,temp_value;
	float z1,z2,z3;
	float inv_z1,inv_z2,inv_z3;
	float tx1_overz,tx2_overz,tx3_overz,ty1_overz,ty2_overz,ty3_overz;
	Color t_color,color1,color2,color3;
	mempitch>>=2;
	UINT* lp_buffer = (UINT*)dest_buffer;
	UINT* lp_addr = NULL;
	...
	x1 = v1->x;y1 = v1->y;z1 = v1->z;inv_z1 = 1/z1;
	x2 = v2->x;y2 = v2->y;z2 = v2->z;inv_z2 = 1/z2;
	x3 = v3->x;y3 = v3->y;z3 = v3->z;inv_z3 = 1/z3;
	tx1 = v1->tx;ty1 = v1->ty;tx1_overz = tx1/z1;ty1_overz = ty1/z1;
	tx2 = v2->tx;ty2 = v2->ty;tx2_overz = tx2/z2;ty2_overz = ty2/z2;
	tx3 = v3->tx;ty3 = v3->ty;tx3_overz = tx3/z3;ty3_overz = ty3/z3;
	if (y3<min_clip_y||y1>max_clip_y||
		(x1<min_clip_x&&x2<min_clip_x&&x3<min_clip_x)||
		(x1>max_clip_x&&x2>max_clip_x&&x3>max_clip_x))
	{
	...
	float dtx_left,dty_left,dtx_right,dty_right;
	float dinvz_left,dtx_overz_left,dty_overz_left,dinvz_right,dtx_overz_right,dty_overz_right;
	int iy1,iy3,loopy,loopx;
	int ixs;
	float y31,y32,y21;
	float loopx_dx;
	...
	//float txs,txe,tys,tye;
	float dxdyl,dzdyl,dxdyr,dzdyr,dy,xs,xe,dzdx,zs,ze,dx,current_z;
	float inv_zs,inv_ze,txs_overz,txe_overz,tys_overz,tye_overz;
	int texture_x;
	int texture_y;
	float tx_step;
	float ty_step;
	...
	case TRI_TYPE_BOTTOM:
		...
		dzdyl = (z2-z1)*y21;
		dzdyr = (z3-z1)*y31;
		dinvz_left = (inv_z2 - inv_z1)*y21;
		dtx_overz_left = (tx2_overz - tx1_overz)*y21;
		dty_overz_left = (ty2_overz - ty1_overz)*y21;
		dinvz_right = (inv_z3 - inv_z1)*y31;
		dtx_overz_right = (tx3_overz - tx1_overz)*y31;
		dty_overz_right = (ty3_overz - ty1_overz)*y31;
		//dtx_left = (tx2 - tx1)*y21;
		//dty_left = (ty2 - ty1)*y21;
		//dtx_right = (tx3 - tx1)*y31;
		//dty_right = (ty3 - ty1)*y31;
		...
		if (y1<=min_clip_y)
		{
			...
			zs += (min_clip_y - y1)*dzdyl;
			ze += (min_clip_y - y1)*dzdyr;
			inv_zs += (min_clip_y - y1)*dinvz_left;
			inv_ze += (min_clip_y - y1)*dinvz_right;
			txs_overz += (min_clip_y - y1)*dtx_overz_left;
			txe_overz += (min_clip_y - y1)*dtx_overz_right;
			tys_overz += (min_clip_y - y1)*dty_overz_left;
			tye_overz += (min_clip_y - y1)*dty_overz_right;
			//txs += (min_clip_y - y1)*dtx_left;
			//txe += (min_clip_y - y1)*dtx_right;
			//tys += (min_clip_y - y1)*dty_left;
			//tye += (min_clip_y - y1)*dty_right;
		...
		else
		{
			...
			zs += (iy1 - y1)*dzdyl;
			ze += (iy1 - y1)*dzdyr;
			inv_zs += (iy1 - y1)*dinvz_left;
			inv_ze += (iy1 - y1)*dinvz_right;
			txs_overz += (iy1 - y1)*dtx_overz_left;
			txe_overz += (iy1 - y1)*dtx_overz_right;
			tys_overz += (iy1 - y1)*dty_overz_left;
			tye_overz += (iy1 - y1)*dty_overz_right;
			/*txs += (iy1 - y1)*dtx_left;
			txe += (iy1 - y1)*dtx_right;
			tys += (iy1 - y1)*dty_left;
			tye += (iy1 - y1)*dty_right;*/
		...
		if(x1>=min_clip_x&&x1<=max_clip_x&&x2>=min_clip_x&&x2<=max_clip_x&&x3>=min_clip_x&&x3<=max_clip_x){
			for (loopy = iy1;loopy<=iy3;loopy++,lp_addr+=mempitch,lp_zaddr+=z_pitch)
			{
				...
				tx_step = (txe_overz - txs_overz)/dx;
				ty_step = (tye_overz - tys_overz)/dx;
				for (loopx = 0;loopx<dx;loopx++)
				{
					current_z = zs + loopx*dzdx;
					if (current_z < *(lp_zaddr+ixs+loopx))
					{
						texture_x = (txs_overz + loopx*tx_step)/(inv_zs + loopx*invz_step)*(texture->width - 1);
						texture_y = (tys_overz + loopx*ty_step)/(inv_zs + loopx*invz_step)*(texture->height - 1);
						temp_color = texture->data[(texture_y<<texture->power_of_two)+texture_x];
						temp_color.r *= light_r;
						temp_color.g *= light_g;
						temp_color.b *= light_b;
				...
				inv_zs += dinvz_left;
				inv_ze += dinvz_right;
				txs_overz += dtx_overz_left;
				txe_overz += dtx_overz_right;
				tys_overz += dty_overz_left;
				tye_overz += dty_overz_right;
				//txs += dtx_left;
				//txe += dtx_right;
				//tys += dty_left;
				//tye += dty_right;
			...
			for (loopy = iy1;loopy<=iy3;loopy++,lp_addr+=mempitch,lp_zaddr+=z_pitch)
			{
				...
				if (clip_xs<min_clip_x)
				{
					clip_txs_overz += (min_clip_x - clip_xs)*tx_step;
					clip_tys_overz += (min_clip_x - clip_xs)*ty_step;
					clip_inv_zs += (min_clip_x - clip_xs)*invz_step;
					clip_xs = min_clip_x;					
				}
				if (clip_xe>max_clip_x)
				{
				...
				for (loopx = 0;loopx<(int)dx;loopx++)
				{
					...
					if (current_z < *(lp_zaddr+ixs+loopx))
					{
						texture_x = (clip_txs_overz + loopx*tx_step)/(clip_inv_zs + loopx*invz_step)*(texture->width - 1);
						texture_y = (clip_tys_overz + loopx*ty_step)/(clip_inv_zs + loopx*invz_step)*(texture->height - 1);
						temp_color = texture->data[(texture_y<<texture->power_of_two)+texture_x];
						temp_color.r *= light_r;
						temp_color.g *= light_g;
						temp_color.b *= light_b;
	...
	case TRI_TYPE_TOP:
		...
		dinvz_left = (inv_z3 - inv_z1)*y31;
		dtx_overz_left = (tx3_overz - tx1_overz)*y31;
		dty_overz_left = (ty3_overz - ty1_overz)*y31;
		dinvz_right = (inv_z3 - inv_z2)*y32;
		dtx_overz_right = (tx3_overz - tx2_overz)*y32;
		dty_overz_right = (ty3_overz - ty2_overz)*y32;
		...
		if(x1>=min_clip_x&&x1<=max_clip_x&&x2>=min_clip_x&&x2<=max_clip_x&&x3>=min_clip_x&&x3<=max_clip_x){
			for (loopy = iy1;loopy<=iy3;loopy++,lp_addr+=mempitch,lp_zaddr+=z_pitch)
			{
				...
				for (loopx = 0;loopx<idx;loopx++)
				{
					...
					if (current_z < (float)(*(lp_zaddr+ixs+loopx)))
					{
						texture_x = (int)((txs_overz + loopx*tx_step)/(inv_zs + loopx*invz_step)*(texture->width - 1));
						texture_y = (int)((tys_overz + loopx*ty_step)/(inv_zs + loopx*invz_step)*(texture->height - 1));
						temp_color = texture->data[(texture_y<<texture->power_of_two)+texture_x];
						temp_color.r *= light_r;
						temp_color.g *= light_g;
						temp_color.b *= light_b;
